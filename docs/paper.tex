\documentclass[aps,prx,reprint,10pt,superscriptaddress]{revtex4-2}
...
Adaptation in fluctuating environments is a central problem in evolutionary biology \cite{kingscollegeRiskUncertaintyEvolutionary1982}. Natural populations rarely experience constant conditions: rather, they face environments that change \emph{unpredictably} over time, often on ecological or evolutionary timescales. In such contexts, if sensing the changes in the environment is infeasible or too costly, it is well established in the ecology and population genetics literature that bet-hedging (i.e. investing only a fraction of the population to each possible phenotype) offers a good alternative to adapt to this fluctuating environment \cite{kussellPhenotypicDiversityPopulation2005}.

In these circumstances our current understanding of evolutionary adaptation suggests that populations tend to evolve toward the strategy which maximizes their mean growth rate \cite{gillespieNaturalSelectionVariances1977} \footnote{More precisely, \cite{gillespieNaturalSelectionVariances1977} argues that what gets maximized is the geometric mean of the offspring number $\mu'$ (where $N(t+\Delta t)=\mu'\cdot N(t)$) but this is equivalent to maximizing the arithmetic mean of the growth rate $\langle\mu\rangle$ as we have defined it in \autoref{eq:avg_growth_rate}.}
...
\begin{figure*}
    \centering
    \caption{Left: average evolved strategy as a function of the initial strategy. Right: Pareto front showing the trade-off between growth and extinction for fixed strategies in the symmetric parameter case.} \label{fig:symmetric_parameters_strategy_and_front}
...
\begin{figure*}
    \centering
    \caption{Dependence of the evolved strategy on population size (left) and mutation probability (right) for symmetric parameters. The same qualitative trends as in the asymmetric case are observed.} \label{fig:symmetric_parameters_dependence}
...
\begin{figure*}
    \centering
    \caption{Comparison between heuristic predictions (left) and simulation results (right) for the symmetric parameter set. The heuristic captures the qualitative behavior but shows larger quantitative deviations.} \label{fig:symmetric_parameters_heuristic}
