...
We begin by displaying in \autoref{fig:supplementary_results_1} (left) the average birth rate $\langle\mu_b\rangle$—which serves as the basis for the heuristic approximation in \autoref{eq:heuristic_distribution}—for the PR parameter set across different initial conditions. We observe that $\langle\mu_b\rangle$ for fixed strategies exhibits a fundamentally different behavior than the total population growth rate, increasing monotonically with $s(A)$ and reaching its maximum at $s(A)=1$. Another key observable of interest is the growth rate fluctuations $\sigma_\mu^2$, computed according to \autoref{eq:std_dev_growth_rate}. Such fluctuations are commonly employed as a proxy for extinction risk \cite{dinisPhaseTransitionsOptimal2020,dinisParetooptimalTradeoffPhenotypic2022}. As shown in \autoref{fig:supplementary_results_1} (right), their behavior qualitatively mirrors the extinction rate, although a simple, exact analytical mapping between the two quantities remains elusive.

Furthermore, we examine the actual phenotype distribution sustained within the population. Although primarily dictated by the strategy, this distribution does not coincide with it due to phenotype-dependent lifetimes. In \autoref{fig:supplementary_results_2} (left), we plot the actual probability of the resistant phenotype $A$, which increases monotonically with $s(A)$. This observable allows for an insightful consistency check: the theoretical $p(A)$ can be analytically derived for static environments from the dominant eigenvector of the matrix
\begin{equation}
    \begin{pmatrix}
    ...
    \end{pmatrix},
\end{equation}
which governs the deterministic population dynamics in a fixed environment $e$. In \autoref{fig:supplementary_results_2} (left), the two dashed lines represent the expected analytical values of $p(A)$ within each of the two isolated environments across the range of fixed strategies. Crucially, the observed phenotype composition under fluctuating environments systematically lies bounded between these two theoretical limits.
