...
\end{equation}
as first argued by \cite{gillespieNaturalSelectionVariances1977}. However, this is not an exact result for finite populations, and when environmental fluctuations are strong and the extinction rate $r_\text{ext}$ (that is, the inverse of the mean extinction time $\tau_\text{ext}$) is non-negligible, as in limited-size populations, we expect to see deviations from this principle.
...
To address the question outlined in the introduction we introduce the simple agent-based model illustrated in \autoref{fig:model_diagram}. The state of the system consists of only two components: the \emph{environment} and the \emph{population}.

The \emph{environment} is simply a discrete variable with two possible values (of course this is easily generalizable to the case of $n_\text{env}$ different environments), $e\in\{1,2\}$, which evolves according to a continuous-time Markov chain with transition rates $\omega_t(e,e')$ between states $e$ and $e'$ \footnote{The possibility that this environment variable follows non-Markovian dynamics or takes continuous values is outside of the scope of this work.}.

The \emph{population} is made up of $N$ agents, each one of which will be characterized just by two traits: a \emph{phenotype} $\phi\in\{A,B\}$ (again this could easily be generalized to more than two phenotypes) and a \emph{phenotypic strategy} $s(\phi)$, which represents a probability distribution over the possible phenotypes. Since we restrict ourselves to two phenotypes, the strategy is fully specified by a single parameter, $s(A)$, as $s(B)=1-s(A)$. Throughout this work, we will therefore specify only this first component. The phenotype will determine the response of the agent to the environment while the phenotypic strategy will determine the phenotype of the offspring of that agent, introducing a mechanism for phenotypic diversity to appear and be maintained \emph{across generations}.
...
\end{equation}
where $\langle\mu_b\rangle$ denotes the average birth rate (that is, the contribution to the growth rate arising from replication events only, excluding deaths) and all quantities are measured in fixed-strategy simulations. To evaluate $\langle\mu\rangle$, $\langle\mu_b\rangle$ and $r_\text{ext}$ for arbitrary values of $\overline{s}$ and $N_\text{ini}$ we use spline interpolations of the numerical data, shown as dashed lines in \autoref{fig:fixed_results_and_heuristic_approx}.
