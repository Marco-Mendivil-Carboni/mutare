...
% To evaluate the predictive power of this framework, \autoref{fig:heuristic_approx} compares the expected average strategy calculated from the heuristic distribution (top) with the actual average observed in full evolutionary simulations (bottom). While a detailed comparison of the full probability distributions is deferred to \autoref{sec:supplementary_results}, focusing on their mean values reveals good qualitative agreement across the entire $N_\text{ini}$ and $p_\text{mut}$ parameter space. In particular, the heuristic successfully captures the transition from growth-maximizing to safer strategies as $N_\text{ini}$ decreases. Notably, in the large-$N_\text{ini}$ limit, the dependence on $p_\text{mut}$ vanishes, confirming that the evolutionary outcome in this regime is governed primarily by growth rather than mutation details.
