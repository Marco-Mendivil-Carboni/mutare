...
In these circumstances our current understanding of evolutionary adaptation suggests that populations tend to evolve toward the strategy which maximizes their mean growth rate \footnote{More precisely, \cite{gillespieNaturalSelectionVariances1977} argued that what gets maximized is the geometric mean of the offspring number $\mu'$ (where $N(t+\Delta t)=\mu'\cdot N(t)$) but this is equivalent to maximizing the arithmetic mean of the growth rate $\langle\mu\rangle$ as we have defined it in \autoref{eq:avg_growth_rate}.}
\begin{equation} \label{eq:avg_growth_rate}
    \langle\mu\rangle=\left\langle\frac{1}{N(t)}\frac{dN}{dt}(t)\right\rangle
\end{equation}
as first argued by \cite{gillespieNaturalSelectionVariances1977}. However, this is not an exact result for finite populations, and when environmental fluctuations are strong and extinction risk is non-negligible, as in limited-size populations, we expect to see deviations from this principle. In particular, we want to investigate whether natural selection under this conditions may favor strategies that sacrifice some average growth rate in exchange for a lower extinction rate $r_\text{ext}$ (that is, the inverse of the mean extinction time $\tau_\text{ext}$).
