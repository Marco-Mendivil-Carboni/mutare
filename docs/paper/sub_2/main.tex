...
\begin{appendices}
    ...
    \begin{figure}
        ...
        \caption{Average population birth rate $\langle\mu_b\rangle$ (top) and growth rate fluctuations $\sigma_\mu$ versus mean growth rate $\langle\mu\rangle$ (bottom) for different initial strategies $s(A)_\text{ini}$ and the different kinds of simulations with the PR parameters.} \label{fig:supplementary_results_1}
    \end{figure}
    ...
    We also examine the actual phenotype distribution within the population. Although dictated primarily by the strategy, this distribution does not coincide with it due to phenotype-dependent lifetimes. In \cref{fig:supplementary_results_2} (top), we plot the actual probability of the resistant phenotype $A$, which increases monotonically with $s(A)$. This observable allows for a useful consistency check: the theoretical $p(A)$ can be analytically derived for static environments and fixed strategies from the dominant eigenvector of the matrix
    \begin{equation}
        \begin{pmatrix}
    ...
    Furthermore, \cref{fig:supplementary_results_3} illustrates how the mean growth rate $\langle\mu\rangle$ (top) and the extinction rate $r_\text{ext}$ (bottom) obtained from evolutionary simulations satisfy the relation proposed in \cref{eq:evol_averages} (represented as dashed lines) for various mutation probabilities $p_\text{mut}$. Quantitative agreement with the simulation data confirms that, as argued in \cref{sec:heuristic_interpretation}, the long-term evolutionary dynamics can be accurately approximated as a sequential concatenation of fixed strategies.

    \begin{figure}
        \includegraphics{asymmetric/prob_mut/avg_growth_rate.pdf}
        \includegraphics{asymmetric/prob_mut/extinct_rate.pdf}
        \caption{Mean population growth rate $\langle\mu\rangle$ (top) and extinction rate $r_\text{ext}$ (bottom) as a function of the mutation probability $p_\text{mut}$ for the PR parameters in \texttt{evol(r)} simulations. The dashed lines indicate the theoretical prediction from \cref{eq:evol_averages}.} \label{fig:supplementary_results_3}
    \end{figure}
    ...
    \begin{figure}
        ...
        \caption{Time-dependent average strategy $\langle\bar{s}\rangle_\tau$ for the PR parameters. Top: Convergence of \texttt{evol} simulations starting from different initial strategies. Bottom: Dependence of $\langle\bar{s}\rangle_\tau$ on the population limitation for \texttt{evol(r)} simulations.}
