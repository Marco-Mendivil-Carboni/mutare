...
\begin{appendices}
    ...
    Next, \cref{fig:supplementary_results_3} illustrates how the mean growth rate $\langle\mu\rangle$ (top) and the extinction rate $r_\text{ext}$ (bottom) obtained from evolutionary simulations satisfy the relation proposed in \cref{eq:evol_averages} (represented as dashed lines) for various mutation probabilities $p_\text{mut}$. Quantitative agreement with the simulation data confirms that, as argued in \cref{sec:heuristic_interpretation}, the long-term evolutionary dynamics can be accurately approximated as a sequential concatenation of fixed strategies.
