...
\begin{appendices}
    ...
    \end{equation}
    rather than simple arithmetic means. All observables considered in this work are computed in this way, with the sole exception of the fluctuations of the population growth rate, which we denote as $\sigma_\mu$ and analyze in \cref{sec:birth_rate_and_fluctuations}. The issue lies in the fact that a naive calculation of the standard deviation based directly on the step-wise rates $\mu_k=\Delta N_k/(N_k\tau_k)$ produces ill-defined, divergent fluctuations due to the arbitrarily small waiting times between the Gillespie steps. The most appropriate way to quantify these fluctuations is through:
    ...
    \begin{figure}
        ...
        \caption{Time-dependent average strategy $\langle\bar{s}\rangle_\tau$ for the PR parameters. Top: Convergence of \texttt{evol} simulations starting from different initial strategies. Bottom: Dependence of $\langle\bar{s}\rangle_\tau$ on the population limitation for \texttt{evol(r)} simulations. The dashed and dotted lines indicate the strategies that maximize mean growth and minimize extinction, respectively.} \label{fig:tau_avg_strat_phe_0}
    \end{figure}
