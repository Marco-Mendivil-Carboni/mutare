...
\end{equation}
where $\langle\mu\rangle(\bar{s})$ and $r_\text{ext}(\bar{s})$ represent the mean growth rate and extinction rate of a strictly \emph{fixed} strategy with value $\bar{s}$ (the baseline blue curve in \cref{fig:pareto_front}), while $p(\bar{s})$ is the probability distribution of the population-averaged strategy $\bar{s}$ recorded over time during the active evolutionary simulations. In \footnote{\label{SupplementalMaterial} See Supplemental Material at [URL to be inserted by publisher] for ...}, we verify that this approximation holds with high accuracy across a wide range of parameters. This fact motivates the search for a heuristic approximation of the distribution $p(\bar{s})$, constructed solely from properties of fixed strategies, that could allow us to predict the evolutionary outcome $\langle\bar{s}\rangle(A)$ as a function of $N_\text{ini}$ and $p_\text{mut}$.
...
Finally, the heuristic approximation still successfully captures this qualitative transition between regimes. However, in this case quantitative discrepancies are more pronounced (see details in \cref{SupplementalMaterial}). In particular, approaching the growth-maximizing strategy requires significantly larger $N_\text{ini}$ values than predicted by the heuristic approximation.
...
\begin{appendices}
    ...
    \end{equation}
    rather than simple arithmetic means. All observables considered in this work are computed this way, with the sole exception of the population growth rate fluctuations, which we denote as $\sigma_\mu$ and analyze in \cref{SupplementalMaterial}. The issue lies in the fact that a naive calculation of the standard deviation based directly on the step-wise rates $\mu_k=\Delta N_k/(N_k\tau_k)$ yields ill-defined, diverging fluctuations due to the arbitrarily small waiting times between Gillespie steps. The most appropriate way to quantify these fluctuations is through:
