...
\begin{frame}
  ...
  \note[item]{\alert{Continuous-time} Markov process describing a population evolving in a fluctuating environment.}
...
\begin{frame}
  ...
  \begin{block}{Simulation types}
    \begin{itemize}
      \item \alert{Fixed}: single prescribed strategy, no evolution ($p_{\text{mut}}=0$).
      \item \alert{Evolutive}: monomorphic initial population, strategies evolve.
      \item \alert{Random}: random initial strategies, then evolve.
    \end{itemize}
  \end{block}
...
\begin{frame}
  ...
  \note[item]{Both monomorphic and random initial populations converge to preety much the same stationary strategy.}
  \note[item]{This indicates the presence of a stable attractor in strategy space.}
  \note[item]{At each instant, the population is nearly monomorphic: the instantaneous dispersion is very small.}
