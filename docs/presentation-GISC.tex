...
\begin{frame}
  \begin{columns}
    ...
    \begin{column}{0.45\textwidth}
      \begin{block}{Evolutionary outcome}
        \begin{itemize}
          \item Convergence to a \alert{well-defined strategy}.
          \item \alert{Independent} of initial conditions.
          \item Population is \alert{nearly strategy-homogeneous} at any time.
        \end{itemize}
      \end{block}
      \begin{block}{Take-home message}
        Evolution selects a \alert{compromise}.
      \end{block}
    \end{column}
    \begin{column}{0.55\textwidth}
      \begin{figure}
  ...
  \note[item]{The histogram however shows broader fluctuations over time and across runs, but with a clear \alert{well-defined mean}.}
  ...
  \note[item]{The \alert{time-series} analysis will clarify how this compromise dynamically emerges.}
  \note[item]{This justifies comparing evolutive simulations with fixed-strategy simulations.}
  \note[item]{The selected strategy lies \alert{between} the growth-maximizing and extinction-minimizing strategies.}
\end{frame}
