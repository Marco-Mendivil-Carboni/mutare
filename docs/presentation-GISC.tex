...
\begin{frame}
  ...
  \note[item]{We will use a minimal mathematical model, not a detailed biological one.}
  \note[item]{To investigate this, we will use numerical simulations.}
  \note[item]{The main objective is to understand how populations adapt in fluctuating environments under demographic constraints.}
  \note[item]{A key question is whether these outcomes can be described using an effective notion of fitness.}
\end{frame}
  ...
  \begin{block}{Variables}
    \begin{table}
      ...
      \begin{tabular}{c c c}
        ...
        symbol & name        & range                          \\
        \midrule
        $e$    & environment & $\{0,\dots,n_{\text{env}}-1\}$ \\
        \addlinespace
        $\phi$ & phenotype   & $\{0,\dots,n_{\text{phe}}-1\}$ \\
        \bottomrule
      \end{tabular}
    \end{table}
  \end{block}
...
\begin{frame}{Numerical methods: Gillespie algorithm}
  ...
  \begin{equation}
    \tau = \frac{1}{a_0(\mathbf{X})}
    \ln\!\left(\frac{1}{r_1}\right),
    \ r_1 \sim U(0,1)
  \end{equation}

  \begin{equation}
    \sum_{j=1}^{\mu-1} a_j(\mathbf{X})
    < r_2 a_0(\mathbf{X})
    \le \sum_{j=1}^{\mu} a_j(\mathbf{X}),
    \ r_2 \sim U(0,1)
  \end{equation}
