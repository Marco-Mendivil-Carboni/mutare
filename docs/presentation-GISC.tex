...
\begin{frame}
  ...
  \begin{block}{Types of simulations}
    \begin{enumerate}
      ...
      \item \alert{\texttt{fixed}}: \alert{shared} initial strategy $s(\phi)_{\text{ini}}$, \alert{not evolving} ($p_{\text{mut}}=0$).
      \item \alert{\texttt{evol}}: \alert{shared} initial strategy $s(\phi)_{\text{ini}}$, \alert{evolving}.
    \end{enumerate}
  \end{block}
    ...
    \begin{column}{0.45\textwidth}
      \begin{block}{Observables}
        \begin{itemize}
          ...
          \item Average phenotypic strategy $\langle s(A)\rangle$
          \item Distribution of phenotypic strategies $p(s(A))$
