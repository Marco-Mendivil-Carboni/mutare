...
\begin{frame}
  ...
  \note[item]{Increasing $N_i$ strongly reduces the \alert{extinction risk} but does \alert{not} change the \alert{average growth rate}.}
  ...
  \note[item]{Notice as well that the \alert{instantaneous dispersion} of strategies is still small, but larger than at small $N_i$.}
...
\begin{frame}
  \begin{block}{Conclusions}
    \begin{itemize}
      ...
      \item \alert{Extinction risk} shapes the evolutionary outcome.
    \end{itemize}
  \end{block}
    ...
    \begin{column}{0.45\textwidth}
      \begin{block}{Future work}
        \begin{itemize}
          ...
          \item Identify an effective optimization principle combining growth and extinction.
          \item Investigate environment-dependent strategies (sensing).
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.55\textwidth}
  ...
  \note[item]{Deriving this optimization principle from theory is a key open problem, and \alert{large deviations theory} appears as a natural framework to address extinction.}
\end{frame}
